...
class DerainCycleGAN(nn.Module):
    def __init__(self, opts):
        ...
        self.myBatchNormlize = myBatchNormlize().to(self.device)
        self.myUnnormlize = myUnormlize().to(self.device)
        ...
        self.criterionRGM = GANLoss('lsgan').to(self.device)
        self.TVloss = Drecloss_stripe().to(self.device)
        self.ms_ssim_mix = MS_SSIM_L1_LOSS().to(self.device)
    ...
    # 将数据放入GPU
    def setgpu(self):
        self.disA.to(self.device)
        self.genA.to(self.device)
        self.genB.to(self.device)
